...
Everybody knows you aren't supposed to serialize in-memory objects to *binary*, we all have scary experiences with `.Rdata` and `.rds`, and Python has the same with its *pickle*. 
...
The same store, opened by four different tools across two languages - there's a recipe, but it's not a nailed down format yet. 

---

*Michael Sumner is a research software engineer at the Australian Antarctic Division. Packages mentioned — `tidync` (rOpenSci reviewed), `vapour`, `vrtstack`, `zaro` — are part of the [hypertidy](https://github.com/hypertidy) ecosystem.*
