...
(Fully [reproducible setup](#appendix-setup), downloading directly from NCEI is in the appendix.)
...
## Appendix: a progressive tour on 23 OISST files {#appendix}

### Setup: download 23 OISST files from NCEI {#appendix-setup}
...
tidync recently got an update in a dev branch to input multiple files, this was a long wishlist that was
out of reach in the early years of the package. I had forgotten also that remote access to NetCDF is possible
via its own library by appending `#mode=bytes` to the URL - this means we don't always need an OpenDAP-enabled
server for remote read. (GDAL uses its own techniques for this remote access via `/vsicurl/` on Linux). 
...
This is an R equivalent of the xarray VirtualiZarr open: the same structure, the same laziness, the same multi-file concatenation. The recipe here is the code rather than a Parquet store - and the `tidync` object it produces is fully navigable with `hyper_filter()` and `hyper_array()` without touching the data.

The serializable recipe (the Parquet store, the multidim VRT) is still the goal for sharing and interoperability. But as a working representation for analysis, this is worth exploring.

### The xarray counterpart — for comparison

First create the VirtualiZarr version. We use the GDAl mdim VRT hosted above because it's convenient, and that's where this effort will start - GDAL already has all the metadata and array concatenating logic, we can pair that with byte reference extraction improvements to GDAL's `GetRawBlockInfo()`. 
...
# 4. serialize — a ~15KB Parquet file describing all 23 files
combined.virtualize.to_kerchunk("sst_virtual.parquet", format="parquet")
```

Now open this with xarray. 
...
#Dimensions:  (time: 23, zlev: 1, lat: 720, lon: 1440)
#Coordinates:
#  * time     (time) datetime64[ns] 184B 2025-01-01T12:00:00 ... 2025-12-19T12...
...
*Michael Sumner is research software engineer at the Australian Antarctic Division. Packages mentioned — `tidync` is rOpenSci reviewed, `vapour`, `vrtstack`, `zaro` — are part of the [hypertidy](https://github.com/hypertidy) ecosystem.*
