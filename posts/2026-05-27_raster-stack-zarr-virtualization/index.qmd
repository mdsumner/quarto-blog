---
title: "R, GDAL, Zarr, and the Missing Recipe Format for Raster Datasets"
...
NOTE: this is very much pseudo code showing the overall style - see the [Usage VirtualiZarr pages](https://virtualizarr.readthedocs.io/en/latest/usage.html) for more examples, and we have a real example in the appendix. 
...
- **Look at `vrtstack` and `zaro`** on the [hypertidy GitHub](https://github.com/hypertidy), vrtstack builds a multidim VRT from 2D rasters (multidim GDAL supports COG, but doesn't support *builiding these*), zaro is a read-implementation for Zarr that is primarily based around Arrow.
...
- **Check out gdalraster (and my multidim branch)** there's a long way to go for multidim but [gdalraster](https://firelab.github.io/gdalraster/) is a rich  package has a super strong basis for faithfully carrying the core facilities of the multidim api [exposed to R](https://github.com/mdsumner/gdalraster/tree/gdalmultidim-api). 
...
```bash
# writeLines(sprintf("/vsicurl/%s", urls), "filelist.txt")
gdal mdim mosaic --input @filelist.txt -o /tmp/sst_mdim.vrt
```

Or from R via `vrtstack` or a direct `system()` call. Now hand that VRT to `read_mdim()`:

```r
stars::read_mdim("/tmp/sst_mdim.vrt", proxy = TRUE)
stars_proxy object with 4 attributes in 4 file(s):
$sst
[1] "[...]/sst_mdim.vrt"
...
23 time steps in `/tmp/sst_mdim.vrt` which is a tiny file of XML. 

This is what `gdal mdim mosaic` unlocks: `read_mdim()` and friends need VRT to make the multi-file case work.
...
Oh, and turns out `terra::rast()` *does* support multidim (not our VRT file sadly): 
