...
Zarr is a chunked array format designed for cloud object storage. Icechunk is a Rust library
sits on top of Zarr and adds git-like snapshots combined with (reliable and guaranteed) [ACID transactions](https://icechunk.io/en/stable/understanding/concepts/#why-transactions-matter). 
...
{{< include ecmwf-aifs-single-info.md >}}
...
`gdal mdim` provides a family of cli support for the [GDAL multidimensional raster model](https://gdal.org/en/stable/user/multidim_raster_data_model.html) - this means that array data
...
GDAL config options can be set through a package wrapper - `gdalraster::set_config_option("AWS_NO_SIGN_REQUEST", "YES")` or `terra::setGDALconfig("AWS_NO_SIGN_REQUEST", "YES")` - or as process environment variables (`Sys.setenv(AWS_NO_SIGN_REQUEST = "YES")`, or `KEY=value` in the shell). Both routes are *global and sticky*: once set they colour every later GDAL call in the session, which is the spooky-action-at-a-distance worth avoiding.

That's why setting config *inline* - `--config KEY=VALUE` on the `gdal mdim info` call above - is the clean option: it lives and dies with that one invocation, a bit like `with` in Python or `with()` in R. The R-side equivalent for env vars and options is `withr::with_envvar()` / `withr::with_options()`, which are true scoped context managers.

With online storage it can be bewildering what exact configuration you need and what you have, but it does come down to a fairly comprehensible set of ideas. This post is not a guide to online storage access configuration, but please be assured that there's a finite set of concepts behind the configuration jungle.
...
**Endpoint, region, hosting-style.** That object is hosted on an *endpoint* - `s3://` is shorthand for `s3.<region>.amazonaws.com`, `gs://` for `storage.googleapis.com`. A *custom* endpoint means pointing at your own host - `AWS_S3_ENDPOINT=your.endpoint.net` - so that `s3` in `s3://bucket/key` resolves to your host and not Amazon's. For non-AWS S3 implementations (MinIO, Ceph) this usually goes together with path-style addressing, `AWS_VIRTUAL_HOSTING=NO`, since most of them don't do virtual-host style. `AWS_REGION` is sometimes demanded even when it oughtn't matter - set it to "".

### GDAL-specific wrinkle: the virtual filesystem
...
This is the most common first stumble: `s3://…` handed to `terra::rast()` or `vapour::gdal_raster_data()` won't open; the `/vsis3/…` form will. The config options from above (`AWS_NO_SIGN_REQUEST`, `AWS_S3_ENDPOINT`, and friends) all still apply unchanged - VSI only changes how you *name* the object, not how you authenticate to it.
...
That listing is a *structural* filesystem — real files physically inside the zip, enumerated out of a remote archive you never downloaded. Append one of them after the closing brace and you've named a single item; GDAL opens it.

Compare that to the subdataset syntax, multiple subdatasets we saw in the Icechunk store above:

```bash
ZARR:"/vsiicechunk/{/vsis3/.../v0.1.0.icechunk}":/dew_point_temperature_2m
```

Again we have bracing of an internal `/vsi*` item but the thing named after it, `/dew_point_temperature_2m`, isn't a file but a variable in a *logical* hierarchy, a node in the Zarr data model, not something to unzip or find at a real url. `/vsizip/` descends into the *structural* contents of an archive, `ZARR:` over `/vsiicechunk/` into the *logical* contents of a data model - and you reach the thing needed. 

GDAL can list the structural contents: 
...
ok we think, so let's go deeper (eek!): 
...
yes it's valid content but not something we can understand - we are seeing the internal structure of a complex sophisticated cloud format. We know it's Icechunk so wield the virtualization tool
