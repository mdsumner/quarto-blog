...
[ximage](https://github.com/hypertidy/ximage) merges the features of both into one function that uses the rasterImage orientation:
...
[fasterize](https://github.com/hypertidy/fasterize) is a high-performance replacement for `raster::rasterize()`. It uses the classic scanline algorithm to burn polygons onto a grid. The original fasterize required an actual RasterLayer as a template—not for its data, but for its six numbers (dimension and extent).
...
[controlledburn](https://github.com/hypertidy/controlledburn) takes the fasterize algorithm and strips away the last step: instead of filling a raster with values, it returns the *indices* of which cells each polygon covers. The output is run-length encoded scanline segments—start, end, row, polygon_id.
...
[grout](https://github.com/hypertidy/grout) applies the same principle to tiling. Given a grid's dimension and extent, plus a block size, it computes the complete tiling scheme: how many tiles, where each one falls, how much overlap ("dangle") occurs when dimensions don't divide evenly.
...
dplyr::glimpse(tile_index(scheme))
```
...
## Python has a similar mix of history, concepts, and tools
