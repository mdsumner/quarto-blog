...
This post attempts to explain the terminology, with a particular stance and this hierarchy:
...
This post is a companion to [Detecting hidden grids in curvilinear coordinates](https://www.hypertidy.org/posts/2026-02-07_cryptic-curvilinear-detection/cryptic-curvilinear), which develops the practical detection problem. Here we make a stance and explain why it matters with real world examples. 
...
# TODO: construct a regular grid and plot cell centres + edges
# vaster::vas(xmin, xmax, ymin, ymax, ncols, nrows)
# show the outer product structure: x[i] ⊗ y[j]
```

The netCDF convention for a regular grid stores 1D coordinate variables - one vector per axis - which is already more than strictly necessary. This is the **degenerate rectilinear** case: the 1D vectors happen to be uniformly spaced, so the four-number specification is sufficient. Many files store them anyway, and many readers treat the uniform case no differently from the variable-spacing case.

What are the coordinates? Well they are the positions of the points above, the centres of a cell. It's trivial to observe that we only need 8 points for every x coordinate, 5 for every y coordinates. But, in turn we don't need the coordinates at all *because they were generated* from 6 numbers.  Those six are extremely general, every regular grid ever is a variation on those values: positive ncols, nrows >= 1; any numbers at all for xmin, xmax, ymin, ymax so long at xmax > xmin; ymax > ymin.  
...
**Note on contested usage:** Some systems (particularly in HPC/CFD) have begun using *rectilinear* to cover both regular and curvilinear cases, blurring the distinction. In this post and the hypertidy ecosystem, *rectilinear* means what it says: rectangle-forming, axis-aligned, independently varying in each dimension. The distinction from regular matters because the storage and the indexing arithmetic differ.
